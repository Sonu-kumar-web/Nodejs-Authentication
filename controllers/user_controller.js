...
module.exports.create = async (req, res) => {
   try {
      ...
      if (req.body.password.length < 6) {
         req.flash("error", "Passwords should not be less than 6 characters.");
         return res.redirect("/users/sign-up");
      }
      ...
      if (!user) {
         ...
         req.flash(
            "success",
            "You are now registered. An e-mail has been sent to " +
               req.body.email +
...
module.exports.verifyEmail = async (req, res) => {
   try {
      ...
      if (!user) {
         req.flash("error", "Wrong Token Cannot Verify User!");
         return res.redirect("/users/sign-in");
      }
      if (user.isVerified) {
      ...
      //user.save();
      req.flash("success", "User Verification Sucessfull!");
      return res.redirect("/users/sign-in");
   } catch (error) {
      console.log(error);
